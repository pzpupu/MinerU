...
(b) In which direction is the directional derivative maximized and how much is it? Solution:
...
and $D\left( {1,2}\right)  = 8, D\left( {-\frac{1}{3},\frac{2}{3}}\right)  =  - 8$ . Then:
...
from which we take ${x}^{3} = {64}$ or $x = 4$ and $y = \frac{64}{{4}^{2}} = 4$ . Plugging these back in ${xyz} = {32}$ we take $z = 2$ . So the dimensions of the box are (4,4,2).
...
7. The total production of a certain product is modeled by the Cobb-Douglas function $P = {100}{L}^{3/4}{K}^{1/4}$ , where $L$ represents the units of labor and $K$ represents the units of capital. Each labor unit costs $\$ {200}$ and each capital unit costs $\$ {250}$ . If the total expenses for labor and capital cannot exceed $\$ {50},{000}$ , find the maximum level of production. Solution:
...
Now from the last two we take $z = 2 - {4x}$ and $z = 2{x}^{2}$ , and by eliminating $z$ we take $x =  - 1 \pm  \sqrt{2}$ . So finally we have two solutions $\left( {-1 - \sqrt{2}, - 1 - \sqrt{2},6 + 4\sqrt{2}}\right)$ and $\left( {-1 + \sqrt{2}, - 1 + \sqrt{2},6 - 4\sqrt{2}}\right)$ . Then
